...
1. Now we have it refactored to see:
   setup: create all commands/events required by tests
   cleanup: cancel all commands/events still running

Moreover:
- we use try .. finally to ensure "however we fail we will cleanup after test
  - note that such layout helps in not missing "things to stop"
    (have you noticed we didn't cancel ping_is_on event?)
- real logic "what the test is all about" is visible

2. add final check after finally:
    if ping_times["lost_connection_time"] == 0 or ping_times["reconnection_time"] == 0:
        MolerTest.error("No outage or not reconnected")
"""
