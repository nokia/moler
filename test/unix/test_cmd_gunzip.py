...
def test_gunzip_raise_error_on_wrong_option(buffer_connection):
    gunzip_cmd = Gunzip(connection=buffer_connection.moler_connection, archive_name=["old.gz"], options='-abc')
    command_output, expected_result = command_output_and_expected_result_on_wrong_option()
    buffer_connection.remote_inject_response([command_output])
    assert 'gunzip -abc old.gz' == gunzip_cmd.command_string
    with pytest.raises(CommandFailure):
        gunzip_cmd()


def test_gunzip_raise_error_on_no_such_file(buffer_connection):
    gunzip_cmd = Gunzip(connection=buffer_connection.moler_connection, archive_name=["new5"])
...
def command_output_and_expected_result_on_wrong_option():
    output = """xyz@debian:~/Dokumenty/sed$ gunzip -b new5.gz
