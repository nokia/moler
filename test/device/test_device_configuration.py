...
def test_clone_device_from_cloned_device_device(moler_config, device_factory):
    conn_config = os.path.join(os.path.dirname(__file__), os.pardir, "resources", "device_config.yml")
    moler_config.load_config(config=conn_config, config_type='yaml')
...
def test_clone_and_foreget_device(moler_config, device_factory):
    ...
    device_by_alias = device_factory.get_device(name=device_cloned_again.alias_name)
    assert device_by_alias == device_cloned_again
    assert device_cloned_again.name != device_cloned_name
    assert device_cloned_again.alias_name == device_cloned_name
