...
class Userdel(GenericUnixCommand):
    ...
    _re_command_error_user_used = re.compile(
        r"userdel:\suser\s(?P<USER>.*)\sis\scurrently\sused\sby\sprocess"
        r"\s(?P<PROCESS>.*)",
        re.IGNORECASE,
    )
...
COMMAND_KWARGS_with_option = {"user": "tmp_user", "options": "--force"}
...
COMMAND_KWARGS_with_force_option = {"user": "tmp_user2", "options": "--force"}
