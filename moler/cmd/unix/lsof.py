        ...
        if self._headers:
            ...
            if len(splitted_values) > 1:
                ...
                for header_index, header in enumerate(self._headers, 0):
                    ...
                    if self._proper_position_value(
                        header_index=header_index, value_position=value_position
                    ):
                        item[header] = value
                        data_index += 1
        ...
        if header_index < len(self._headers):
            ...
            if current_header_pos == value_position:
                ...
                if header_index < len(self._headers) - 1:
                    next_header_pos = self._header_pos[header_index] + len(
                        self._headers[header_index]
                    )
                if (
                    value_position > prev_header_pos
                    and value_position < next_header_pos
                ):
                    ret = True
        return ret
...
COMMAND_KWARGS_number_only = {"options": "| wc -l"}

COMMAND_RESULT_number_only = {"NUMBER": 15695, "VALUES": []}
