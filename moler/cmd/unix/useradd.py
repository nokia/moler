...
class Useradd(GenericUnixCommand):
    def __init__(self, connection, prompt=None, new_line_chars=None, runner=None, options=None, defaults=False,
                 user=None):
        super(Useradd, self).__init__(connection=connection, prompt=prompt, new_line_chars=new_line_chars, runner=runner)
    ...
    _re_error = re.compile(r"Usage:\suseradd\s\[options\]\sLOGIN(?P<ERROR>.*)", re.IGNORECASE)
