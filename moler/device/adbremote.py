...
class AdbRemote(UnixRemote):
    ...
    def _overwrite_prompts(self):
        ...
        """
        super(AdbRemote, self)._overwrite_prompts()
        hops_config = self._configurations[AdbRemote.connection_hops]
    ...
    def _prepare_state_hops_with_proxy_pc(self):
        ...
        state_hops = {
            ...
            AdbRemote.adb_shell_root: {
                AdbRemote.not_connected: AdbRemote.adb_shell,
    ...
    def _get_packages_for_state(self, state, observer):
        ...
        if not available:
            if (state == AdbRemote.adb_shell) or (state == AdbRemote.adb_shell_root):
                available = {AdbRemote.cmds: ['moler.cmd.unix'],
                             AdbRemote.events: ['moler.events.shared']}
            if available:
                return available[observer]
        elif state == AdbRemote.unix_remote:  # this is unix extended with adb commands
            if observer == AdbRemote.cmds:
