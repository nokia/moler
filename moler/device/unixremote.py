...
class UnixRemote(ProxyPc):
    ...
    def __init__(
        ...
        """
        initial_state = (
            initial_state if initial_state is not None else UnixRemote.unix_remote
        )
        super(UnixRemote, self).__init__(
            name=name,
    ...
    def _prepare_state_prompts_with_proxy_pc(self):
        ...
        """
        state_prompts = {
            UnixRemote.unix_remote: self._configurations[UnixRemote.connection_hops][
                UnixRemote.proxy_pc
            ][UnixRemote.unix_remote]["command_params"]["expected_prompt"],
            UnixRemote.unix_remote_root: self._configurations[
                UnixRemote.connection_hops
            ][UnixRemote.unix_remote][UnixRemote.unix_remote_root]["command_params"][
                "expected_prompt"
            ],
        }
    ...
    def _prepare_state_prompts_without_proxy_pc(self):
        ...
        state_prompts = {
            ...
            UnixRemote.unix_local: self._configurations[UnixRemote.connection_hops][
                UnixRemote.unix_remote
            ][UnixRemote.unix_local]["command_params"]["expected_prompt"],
        }
        return state_prompts
    ...
    def _prepare_newline_chars_with_proxy_pc(self):
        ...
        newline_chars = {
            UnixRemote.unix_remote: self._configurations[UnixRemote.connection_hops][
                UnixRemote.proxy_pc
            ][UnixRemote.unix_remote]["command_params"]["target_newline"],
            UnixRemote.unix_remote_root: self._configurations[
                UnixRemote.connection_hops
            ][UnixRemote.unix_remote][UnixRemote.unix_remote_root]["command_params"][
                "target_newline"
    ...
    def _prepare_newline_chars_without_proxy_pc(self):
        ...
        newline_chars = {
            ...
            UnixRemote.unix_local: self._configurations[UnixRemote.connection_hops][
                UnixRemote.unix_remote
            ][UnixRemote.unix_local]["command_params"]["target_newline"],
            UnixRemote.unix_remote_root: self._configurations[
                UnixRemote.connection_hops
            ][UnixRemote.unix_remote][UnixRemote.unix_remote_root]["command_params"][
                "target_newline"
    ...
    def _get_packages_for_state(self, state, observer):
        ...
        if not available:
            if state == UnixRemote.unix_remote or state == UnixRemote.unix_remote_root:
                available = {
                    UnixRemote.cmds: ["moler.cmd.unix"],
